...
public class AttachedPathScript : MonoBehaviour 
{
	...
	public void NewPath()
	{
		...
		terrainObj = GameObject.Find(parentTerrain);
	...
	public void CreatePath(int smoothingLevel, bool flatten, bool road)
	{
		...
		for (int i = 0; i < n; i++) 
		{
			...
			for (int j = 0; j < smoothingLevel + 1; j++) 
			{
				...
				tC.position.x = (tweenPoint.x/terData.size.x) * terData.heightmapResolution;
				tC.position.y = (tweenPoint.z/terData.size.z) * terData.heightmapResolution;
				tC.heightAtCell = tweenPoint.y/terData.size.y;
				pathCells.Add(tC);
				...
				tweenPoint.y = terrainHeights[(int)(((float)tweenPoint.z/terData.size.z) * terData.heightmapResolution), (int)(((float)tweenPoint.x/terData.size.x) * terData.heightmapResolution)] * terData.size.y;

				// create vertices at the perpendicular points
				newVertices[nextVertex] = tweenPoint + extrudedPointR;
				if(!road)
					newVertices[nextVertex].y = (((float)terrainHeights[(int)(((float)newVertices[nextVertex].z/terData.size.z) * terData.heightmapResolution), (int)(((float)newVertices[nextVertex].x/terData.size.x) * terData.heightmapResolution)] * terData.size.y) + newVertices[nextVertex-2].y)/2f;
				else
					newVertices[nextVertex].y = (float)terrainHeights[(int)(((float)newVertices[nextVertex].z/terData.size.z) * terData.heightmapResolution), (int)(((float)newVertices[nextVertex].x/terData.size.x) * terData.heightmapResolution)] * terData.size.y;
		...
		newVertices[0].y = terrainHeights[(int)(((float)newVertices[0].z/terData.size.z) * terData.heightmapResolution), (int)(((float)newVertices[0].x/terData.size.x) * terData.heightmapResolution)] * terData.size.y;
		newVertices[1] = nodeObjects[0].position + extrudedPointL;
		newVertices[1].y = terrainHeights[(int)(((float)newVertices[1].z/terData.size.z) * terData.heightmapResolution), (int)(((float)newVertices[1].x/terData.size.x) * terData.heightmapResolution)] * terData.size.y;
		
		if(road)
		{
			for(int i = 0; i < newVertices.Length; i++)
			{
				newVertices[i].y = terComponent.SampleHeight(newVertices[i]) + (0.05f);
			}
		}
...
//		newMesh.RecalculateNormals();
	...
	public void FinalizePath()
	{
		...
		if(terData.alphamapLayers > pathTexture)
		{
			...
			foreach(TerrainPathCell tC in terrainCells)
			{
				...
				Ray pathRay = new Ray(new Vector3((float)(tC.position.x/terData.heightmapResolution) * terData.size.x, (float)tC.heightAtCell * terData.size.y, (float)(tC.position.y/terData.heightmapResolution) * terData.size.z), -Vector3.up);
				
				if(pathCollider.Raycast(pathRay, out raycastHit, 100000))
				{
			...
			*/
			...
			if(!isRoad)
			{
				...
				foreach(TerrainPathCell tC in totalPathVerts2)
				{
					...
					{
						float distanceFromPath = (float)Mathf.Sqrt((float)Mathf.Pow((pV.position.x - tC.position.x), 2f) + Mathf.Pow((pV.position.y - tC.position.y), 2f));
		...
		{
			Debug.Log("Invalid texture prototype");
		}
	}
